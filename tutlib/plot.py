...
def plot_ternary(dataset, components, labels='labels', include_surface=True, surface_data='acquisition', show=True,
                 next_point=None, surface_colorbar=True):
    layout = dict(
        width=750,
        ternary=dict(
            sum=1,
...
def make_boundary_plots(score_dict):
    ...
    out = {}
    out['GT'] = go.Scatter(x=gt_xy.T[0], y=gt_xy.T[1], mode='lines+markers', line={'color': 'black'},
                           marker={'symbol': 'circle-open'}, name='GT')
    out['AL'] = go.Scatter(x=xy.T[0], y=xy.T[1], mode='lines+markers', line={'color': 'green'},
                           marker={'symbol': 'triangle-up-open'}, name='AL')
